...
5. 对数量关系：必须换成不同生活/工作场景，至少改变两个条件关系；不要沿用原题百分数、金额、主体、求解对象和方程结构。
   ...
   - 如果原题是“给出三个事件A/B/C的条件概率，分别计算三者发生概率并排序”，新题禁止继续使用“三个事件、同样的条件依赖链、最后按概率排序”的结构。
   - 可以迁移到利润率、成本构成、销量变化、售价折扣、固定成本摊销、工程效率、浓度混合等其他数量关系外壳，但解题入口必须明显不同。
6. 对概率题：只保留“条件概率、全概率、分类讨论”能力；必须改变事件数量、条件层级或最终问题。
   - 可以改成“至少发生一个事件的概率”“恰好发生两个事件的概率”“某个结果来自哪一路径的概率”“已知结果反推条件概率”等。
   - 不要继续让学生分别求三个对象的边际概率并排序。
   - 如果原题使用A/B/C三个方案、套餐、项目或选项，新题不要再使用A/B/C或甲/乙/丙三个并列对象；应改成两阶段流程、抽检路径、用户转化链、诊断筛查、抽样放回等明显不同的概率结构。
7. 对资料分析：必须换资料背景和指标口径，不能沿用原材料数值或年份；保留同类统计计算能力即可。
8. 选项必须互斥且只有一个正确答案，答案只能是 A/B/C/D。
9. 解析必须先说明“原题考点迁移到哪里”，再给出新题的独立解题路径。
